...
type DistributeArray<T extends string[]> = T extends [infer First extends string, ...infer Rest extends string[]] ? First | DistributeArray<Rest> : never;
type DistributeArrayTest = DistributeArray<['a', 'b', 'c']>;
...
type BEM<B extends string, E extends string[], M extends string[]> = E extends [] ? 
  ...
  M extends [] ? 
    // M 은 [], E 는 string[]
    `${B}__${DistributeArray<E>}` :
  // 둘다 string[]
  `${B}__${DistributeArray<E>}--${DistributeArray<M>}`;
