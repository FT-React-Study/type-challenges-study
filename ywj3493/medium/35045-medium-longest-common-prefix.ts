...
type LongestCommonPrefix<T extends string[], Std extends string = T[0], P extends string = '', Answer extends string = ''> = 
  Std extends `${infer First}${infer Rest}` ?
    // 기준 문자가 '' 이 아닐 때,
    T[number] extends `${P}${string}` ?
      LongestCommonPrefix<T, Rest, `${P}${First}`, P> :
      Answer :
    // 기준 문자가 '' 일 때,
    T[number] extends `${P}${string}` ?
      P :
