...
type IsEqual<X, Y> = (<T>() => T extends Y ? 1 : 2) extends (<T>() => T extends X ? 1 : 2) ? true : false;

type Has<T extends unknown[], G> = T extends [infer F, ...infer R] ? IsEqual<F, G> extends true ? true : Has<R, G> : false;

type CheckRepeatedTuple<T extends unknown[]> = T extends [infer F, ...infer R] ?
  Has<R, F> extends true ? true : CheckRepeatedTuple<R> : false;
