...
type MyOmit<T, U> = {[K in keyof T as K extends U ? never : K]: T[K]}

type DeepOmit<T, U extends string> = `${U}` extends `${infer F extends string & keyof T}.${infer R}` ?
  {[K in keyof T]: K extends F ? DeepOmit<T[K], R> : T[K]} :
  MyOmit<T, U>;

type Test = DeepOmit<obj, 'person'>
